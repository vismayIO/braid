# Implementation Plan: `braid` POC v0.1 (Draft v2)

> **v2 changelog:** Applied all 8 must-fix items from Architect + Critic review of v1 (Step 0 renumbered; Step 6 phase-2 provider fallback chain; Step 4 MemoryClient interface with two backends; R1 mitigation clarified; Section 3 Option A justification strengthened; AC-3 timing assertion concretized; AC-6 tightened; AC-7 + AC-8 explicit verification steps in Section 8; `BRAID_ALLOW_PAID` gate in Step 3; second e2e test for merge coverage; manual gate flagged separately; hello-probe quota caveat noted).

> Deliberate mode. Source spec: `.omc/specs/deep-interview-braid.md` (ambiguity 11%).
